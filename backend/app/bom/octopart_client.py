...
if not NEXAR_API_KEY:
    logger.warning("NEXAR_API_KEY environment variable not set. Octopart client will not be able to authenticate.")
...
async def find_part_by_mpn(mpn: str, sema: asyncio.Semaphore):
    ...
    if CACHE_ENABLED and cache_path.exists():
        ...
        try:
            ...
            logger.warning(f"Could not read cache file for '{mpn}': {e}. Fetching from API.")
    ...
    async with sema, httpx.AsyncClient() as client:
        try:
            ...
            response = await client.post(NEXAR_API_URL, json=payload, headers=headers, timeout=20.0)
            ...
            logger.error(f"HTTP error for {mpn}: {e.response.status_code} - {e.response.text}")
        except httpx.RequestError as e:
            logger.error(f"Network error for {mpn}: {e}")
        except Exception as e:
            logger.error(f"Unexpected error in Octopart client for {mpn}: {e}", exc_info=True)
